...
minetest.register_alias("chest", "chest:chest")
minetest.register_alias("default:chest","chest:chest")


function chest.get_hotbar_bg(x,y)
	local out = ""
	for i=0,7,1 do
...
function chest.register_chest(prefixed_name, d)
	...
	if def.protected then
		...
		def.can_dig = function(pos,player)
			...
			return inv:is_empty("main") and
					chest.can_interact_with_node(player, pos)
		end
		def.allow_metadata_inventory_move = function(pos, from_list, from_index,
				to_list, to_index, count, player)
			if not chest.can_interact_with_node(player, pos) then
				return 0
			end
			return count
		end
		...
		def.on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			if not chest.can_interact_with_node(clicker, pos) then
				return itemstack
			end
		...
		def.on_blast = function(pos)
			local drops = {}
			chest.get_inventory_drops(pos, "main", drops)
			drops[#drops+1] = name
	...
	if name == "chest:chest" or name == "chest:chest_locked" then
		minetest.register_lbm({
			label = "update chests to opening chests",
			name = "chest:upgrade_" .. name:sub(9,-1) .. "_v2",
			nodenames = {name},
...
chest.register_chest("chest:chest", {
	...
	--sounds = chest.node_sound_wood_defaults(),
...
chest.register_chest("chest:chest_locked", {
...
minetest.register_craft( {
	type = "shapeless",
	output = "chest:chest_locked",
	recipe = {"chest:chest", steel_ingot},
})
